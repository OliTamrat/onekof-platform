...
buildGuide({
  ...
  body: [
    ...
    table([
      ...
      ['Choose the organization type at sign-up', 'The person creating the workspace'],
      ['Change the organization type / customize sections later', 'Org Owner or Admin (others can open the page but saving is refused)'],
    ...
    BOLDLEAD('Organization preset (edition) — ', 'a configuration for a type of institution: Ministry / Government, NGO / Non-Profit, Business / Startup, Education, or Healthcare. Chosen at sign-up, it decides which sections appear and which features are on. Changeable later by an Owner or Admin.'),
    ...
    STEP('steps1', 'Create your workspace: name, web address (auto-suggested), organization type, and team size. You land on the Dashboard.'),
    STEP('steps1', 'The organization type — Government/Ministry, NGO/Non-Profit, Business/Startup, Education, Healthcare, or Other — decides which sections appear. Government and NGO workspaces also get formal "Work Cycle" vocabulary instead of "Sprint". Nothing is locked: an Owner or Admin can change any of it later.'),
    ...
    H2('4.3 Change the organization type or fine-tune sections (Owner/Admin)'),
    STEP('steps2', 'Go to Settings → Customization.'),
    STEP('steps2', 'Under Quick Start Presets, choose Ministry / Government, NGO / Non-Profit, Business / Startup, Education, or Healthcare. The sections and features adjust immediately in the preview.'),
    ...
    BOLDLEAD('Presets are starting points, not locks. ', 'Choosing Ministry, NGO, Business, Education, or Healthcare just sets the toggles; an Owner/Admin can change any individual toggle afterwards.'),
    BOLDLEAD('Every workspace is configured from the moment it exists. ', 'The chosen edition is written together with the workspace itself, so a new organization is never left in an unconfigured state.'),
    ...
    table([
      ...
      ['"Where do I choose Ministry / NGO / Healthcare?"', 'At sign-up: the workspace step asks what kind of organization it is, and the matching edition applies immediately. To change it later, an Owner/Admin uses Settings → Customization.'],
      ['"Our workspace was created before that question existed."', 'Its admins see a one-time banner on the Dashboard inviting them to choose the organization type. Until they do, the workspace keeps the general-purpose (Business) configuration — nothing is lost or hidden.'],
    ...
    B('The organization type chosen at sign-up is stored on the organization and its settings row is created in the same database transaction, so no workspace exists without a configuration. Organizations predating this step have no type and fall back to the general-purpose configuration until an admin chooses one.'),
    ...
    table([
      ...
      ['Change org type / toggle sections', 'Settings → Customization — Owner/Admin'],
