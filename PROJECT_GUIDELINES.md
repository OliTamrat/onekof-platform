...
| Sidebar editions Q1: `compliance` and `resources` pages | E1–E4 shipped 2026-07-31 (S1–S9 APPROVED). These two pages stay **unwired by design**: both need a scope decision first — compliance overlaps the audit-log work, resources overlaps Teams/Budget (doc §6 Q1). They are honest placeholders reachable by URL only | Founder answers Q1: build as modules, fold into existing surfaces, or delete the pages |
| Sidebar editions Q3: Education is the thinnest edition | No analytics, no operations — possibly intentional for schools, possibly an oversight from when the preset was written. E2 changed Education's *emphasis* only; membership untouched | Founder answers Q3 |
