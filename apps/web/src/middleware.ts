...
function addSecurityHeaders(response: NextResponse, pathname: string) {
  ...
  const csp = [
    ...
    "font-src 'self' data:",
    `connect-src 'self' ${oauthDomains} https://*.upstash.io https://cloudflareinsights.com https://static.cloudflareinsights.com https://*.sentry.io${allowVercelLive ? ' https://vercel.live https://*.vercel.live' : ''}`,
