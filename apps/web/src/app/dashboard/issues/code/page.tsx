...
import { Plus, GitBranch, GitCommit, GitPullRequest, Code, BarChart3, FileText, Clock, Book, X, Github, GitlabIcon as Gitlab } from 'lucide-react';
...
const TAB_ITEMS = [
  ...
  { id: 'code', label: 'Code', icon: Code, href: '/dashboard/issues/code', active: true },
      ...
      <div className="flex h-full flex-col bg-gray-50 dark:bg-[#1B1F23]">
        ...
        <div className="border-b border-gray-200 dark:border-[#2C333A] bg-white dark:bg-[#22272B]">
          ...
          <div className="flex items-center justify-between border-b border-gray-200 dark:border-[#2C333A] px-6 py-3">
            ...
            <button onClick={() => setShowConnectModal(true)} className="flex items-center gap-2 rounded-md bg-[#0065FF] px-4 py-1.5 text-sm font-medium text-white hover:bg-[#0052CC]">
        ...
        <div className="flex-1 flex items-center justify-center p-6">
          <div className="max-w-2xl text-center">
            ...
            <p className="text-gray-600 dark:text-[#9FADBC] mb-6">
              Link your GitHub, GitLab, or Bitbucket repository to track commits, pull requests, and branches alongside your issues.
            </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-8">
              <div className="p-6 bg-white dark:bg-[#22272B] rounded-lg border border-gray-200 dark:border-[#2C333A]">
                ...
                <p className="text-sm text-gray-600 dark:text-[#9FADBC]">
                  Track code commits linked to issues
                </p>
              </div>
              ...
              <div className="p-6 bg-white dark:bg-[#22272B] rounded-lg border border-gray-200 dark:border-[#2C333A]">
                ...
                <p className="text-sm text-gray-600 dark:text-[#9FADBC]">
                  Monitor PRs and code reviews
                </p>
              </div>

              <div className="p-6 bg-white dark:bg-[#22272B] rounded-lg border border-gray-200 dark:border-[#2C333A]">
                <GitBranch className="h-8 w-8 text-[#0065FF] mb-3" />
                <h3 className="font-semibold text-gray-900 dark:text-white mb-2">Branches</h3>
                <p className="text-sm text-gray-600 dark:text-[#9FADBC]">
                  View active feature branches
                </p>
              </div>
            </div>

            <button onClick={() => setShowConnectModal(true)} className="mt-8 flex items-center gap-2 mx-auto rounded-md bg-[#0065FF] px-6 py-2.5 text-sm font-medium text-white hover:bg-[#0052CC]">
              <Plus className="h-4 w-4" />
              Connect Repository
            </button>
          </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
            <div className="w-full max-w-md bg-white dark:bg-[#22272B] rounded-lg shadow-xl">
              ...
              <div className="p-6 space-y-4">
                <p className="text-sm text-gray-600 dark:text-[#9FADBC]">Choose your code hosting provider</p>
                <div className="space-y-3">
                  <button onClick={() => setSelectedProvider('github')} className={`w-full flex items-center gap-3 p-4 rounded-lg border-2 transition-colors ${selectedProvider === 'github' ? 'border-[#0065FF] bg-[#0065FF]/5' : 'border-gray-200 dark:border-[#2C333A]'}`}>
                  ...
                  <button onClick={() => setSelectedProvider('gitlab')} className={`w-full flex items-center gap-3 p-4 rounded-lg border-2 transition-colors ${selectedProvider === 'gitlab' ? 'border-[#0065FF] bg-[#0065FF]/5' : 'border-gray-200 dark:border-[#2C333A]'}`}>
                  ...
                  <button onClick={() => setSelectedProvider('bitbucket')} className={`w-full flex items-center gap-3 p-4 rounded-lg border-2 transition-colors ${selectedProvider === 'bitbucket' ? 'border-[#0065FF] bg-[#0065FF]/5' : 'border-gray-200 dark:border-[#2C333A]'}`}>
              ...
              <div className="flex items-center justify-end gap-3 p-6 border-t border-gray-200 dark:border-[#2C333A]">
                <button onClick={() => setShowConnectModal(false)} className="px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-[#282E33] rounded-md">Cancel</button>
                <button onClick={handleConnect} disabled={!selectedProvider} className="px-4 py-2 text-sm font-medium text-white bg-[#0065FF] hover:bg-[#0052CC] rounded-md disabled:opacity-50 disabled:cursor-not-allowed">Connect</button>
              </div>
            </div>
          </div>
        )}
