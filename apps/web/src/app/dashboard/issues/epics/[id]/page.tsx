...
const STATUS_META: Record<string, { label: string; icon: any; color: string; bg: string }> = {
  BACKLOG: { label: 'Backlog', icon: Circle, color: 'text-slate-500', bg: 'bg-slate-100 dark:bg-[#181D23]' },
...
export default function EpicDetailPage() {
  ...
  if (isLoading) {
    return (
      <AppLayout>
        <div className="bg-gray-50 dark:bg-[#0B0E11] min-h-full p-6 space-y-4">
      ...
      <div className="min-h-full bg-gray-50 dark:bg-[#0B0E11]">
          ...
          <div className="px-4 md:px-6 py-4">
            ...
            <div className="flex items-center gap-2 mb-3">
              <Button
                ...
                size="sm"
                onClick={() => router.back()}
                className="h-7 px-2 text-xs text-gray-600 dark:text-white/50"
              >
              ...
              <span className="text-xs font-mono text-gray-500 dark:text-white/50">{epic.key}</span>
            </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                {epic.description && (
                  <p className="mt-2 text-sm text-gray-600 dark:text-white/50 max-w-3xl">
                    {epic.description}
                  </p>
                )}
              </div>
        ...
        <div className="px-4 md:px-6 py-6 space-y-6 max-w-6xl">
          ...
          <div>
            ...
            <div className="h-3 w-full rounded-full bg-gray-200 dark:bg-[#181D23] overflow-hidden">
              <div
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <div className="rounded-lg border border-gray-200 dark:border-white/[0.08] bg-white dark:bg-[#12161B] px-4 py-3">
              <div className="text-xs text-gray-500 dark:text-white/50">Total tasks</div>
              <div className="text-2xl font-semibold text-gray-900 dark:text-white mt-1">{stats.total}</div>
            </div>
            <div className="rounded-lg border border-gray-200 dark:border-white/[0.08] bg-white dark:bg-[#12161B] px-4 py-3">
              <div className="text-xs text-gray-500 dark:text-white/50">In progress</div>
            ...
            <div className="rounded-lg border border-gray-200 dark:border-white/[0.08] bg-white dark:bg-[#12161B] px-4 py-3">
              <div className="text-xs text-gray-500 dark:text-white/50">Estimate</div>
              <div className="text-2xl font-semibold text-gray-900 dark:text-white mt-1">
                {stats.estimateTotal}h
              </div>
            </div>
            <div className="rounded-lg border border-gray-200 dark:border-white/[0.08] bg-white dark:bg-[#12161B] px-4 py-3">
              <div className="text-xs text-gray-500 dark:text-white/50">Time spent</div>
              <div className="text-2xl font-semibold text-gray-900 dark:text-white mt-1">
          ...
          <div>
            ...
            {children.length === 0 ? (
              <div className="rounded-lg border border-dashed border-gray-300 dark:border-white/[0.08] bg-white dark:bg-[#12161B] p-8 text-center">
                  ...
                  return (
                    <div key={status}>
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="text-xs font-semibold uppercase tracking-wide text-gray-600 dark:text-white/50">
                          {meta.label}
                        </span>
                      ...
                      <div className="space-y-1.5">
                        {tasks.map((task) => (
                          <button
                            ...
                            className="w-full flex items-center gap-3 rounded-md border border-gray-200 dark:border-white/[0.08] bg-white dark:bg-[#12161B] px-3 py-2 text-left hover:border-primary-500 transition-colors"
                          >
                            <span className="text-xs font-mono text-gray-500 dark:text-white/50 shrink-0 w-20">
                              {task.key}
                            </span>
