...
export async function POST(_request: NextRequest) {
  try {
    ...
    // Category mapping
    const categories = {
      personnel: project.budget.categories.find((c: { code: string | null }) => c.code === '5210'),
      construction: project.budget.categories.find((c: { code: string | null }) => c.code === '5220'),
      equipment: project.budget.categories.find((c: { code: string | null }) => c.code === '5230'),
      environmental: project.budget.categories.find((c: { code: string | null }) => c.code === '5240'),
      management: project.budget.categories.find((c: { code: string | null }) => c.code === '5250'),
      contingency: project.budget.categories.find((c: { code: string | null }) => c.code === '5260'),
    };
