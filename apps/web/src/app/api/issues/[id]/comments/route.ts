...
export async function POST(
  ...
  try {
    ...
    // requireTaskAccess also enforces the M2 care-item rule: a comment thread
    // on a care item discloses everything the task itself would, and is a
    // write into that patient's record besides.
    const access = await requireTaskAccess(params.id, user.id);
