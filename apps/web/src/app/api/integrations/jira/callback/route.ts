...
import { connectJira } from '@/lib/integrations/jira';
...
export async function GET(req: NextRequest) {
  try {
    ...
    await connectJira(state.organizationId, state.userId, code);

    const redirectUrl = sanitizeRedirectUrl(state.redirectUrl, '/dashboard/settings/integrations');
    return NextResponse.redirect(
      new URL(`${redirectUrl}?connected=jira`, req.nextUrl.origin)
    );
  } catch (error) {
    logger.error('Jira callback error', { error: error instanceof Error ? error.message : error });
    return NextResponse.redirect(
