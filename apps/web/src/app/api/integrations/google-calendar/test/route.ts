...
export async function POST() {
  try {
    ...
    const result = await testGoogleCalendarConnection(ctx.organizationId);
    return NextResponse.json(result);
  } catch (error) {
    logger.error('Google Calendar test error', { error: error instanceof Error ? error.message : error });
