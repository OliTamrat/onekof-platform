...
export async function GET(req: NextRequest) {
  try {
    ...
    const connection = await getConnection(ctx.organizationId, 'slack');
...
export async function PUT(req: NextRequest) {
  try {
    ...
    if (notifications) {
      await updateSlackNotifications(ctx.organizationId, notifications);
    }

    if (Object.keys(configUpdates).length > 0) {
      await updateSlackConfig(ctx.organizationId, configUpdates);
    }
...
export async function DELETE() {
  try {
    ...
    await disconnectSlack(ctx.organizationId);
    return NextResponse.json({ success: true });
  } catch (error) {
