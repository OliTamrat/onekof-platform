  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <section id="features" className="py-24 sm:py-32">
        <div className="mx-auto max-w-7xl px-5 sm:px-6 lg:px-8">
          <Reveal>
            <div className="mx-auto mb-16 max-w-2xl text-center">
              <div className="mb-4 inline-flex items-center gap-2 rounded-full bg-brand-50 px-3.5 py-1.5 border border-brand-100">
                <Sparkles className="h-3.5 w-3.5 text-brand-500" /><span className="text-xs font-semibold uppercase tracking-wider text-brand-600">Features</span>
              </div>
              <h2 className="mb-4 text-3xl font-extrabold tracking-tight text-midnight-800 sm:text-4xl lg:text-5xl">
                Everything your team needs.{' '}<span className="bg-gradient-to-r from-brand-600 to-purple-600 bg-clip-text text-transparent">Nothing it doesn&apos;t.</span>
              </h2>
              <p className="text-lg text-midnight-400">Purpose-built for Ethiopian organizations with features you won&apos;t find in any other tool.</p>
            </div>
          </Reveal>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            <Reveal delay={100} className="sm:col-span-2 lg:col-span-2">
              <div className="gradient-border card-tilt group relative overflow-hidden rounded-2xl bg-gradient-to-br from-brand-50 via-surface-50 to-white p-8 shadow-sm hover:shadow-brand-md">
                <div className="relative z-10">
                  ...
                  <p className="max-w-md text-sm leading-relaxed text-midnight-400">The only project management tool with native Ethiopian calendar support. Plan sprints around Pagume, set deadlines in Meskerem, and track milestones the way you think about time.</p>
                </div>
                <div className="absolute -right-4 -bottom-4 hidden rounded-2xl border border-brand-100 bg-white p-4 shadow-brand-md opacity-80 transition-all group-hover:opacity-100 sm:block">
                  ...
                  <div className="grid grid-cols-7 gap-1">
                    {['ሰ','ማ','ረ','ሐ','አ','ቅ','እ'].map((d) => (<div key={d} className="text-center text-[9px] font-medium text-midnight-300">{d}</div>))}
                    {Array.from({ length: 30 }, (_, i) => (
                      <div key={i} className={`h-5 w-5 rounded text-center text-[9px] leading-5 ${i === 13 ? 'bg-brand-600 font-bold text-white' : i === 20 ? 'bg-accent-50 font-medium text-accent-600' : 'text-midnight-500 hover:bg-brand-50'}`}>{i + 1}</div>
                    ))}
                  </div>
                </div>
              </div>
            ...
            <Reveal delay={200}>
              <div className="gradient-border card-tilt group rounded-2xl bg-white p-8 shadow-sm hover:shadow-brand-md">
                ...
                <div className="mt-4 flex flex-wrap gap-1.5">
                  {['English','አማርኛ','Oromoo','ትግርኛ'].map((l) => (<span key={l} className="rounded-full bg-brand-50 border border-brand-100 px-2.5 py-1 text-[10px] font-medium text-brand-700">{l}</span>))}
                </div>
              </div>
            </Reveal>
            ...
            <Reveal delay={250} className="sm:col-span-2">
              <div className="gradient-border card-tilt group relative overflow-hidden rounded-2xl bg-gradient-to-br from-amber-50/50 via-white to-white p-8 shadow-sm hover:shadow-brand-md">
                ...
                <div className="mt-6 flex items-center gap-3">
                  {[{ label: 'When task created', icon: '⚡' },{ label: 'Assign to team lead', icon: '👤' },{ label: 'Notify on Slack', icon: '🔔' }].map((step, i) => (
                    <div key={step.label} className="flex items-center gap-3">
                      <div className="rounded-lg border border-surface-300 bg-white px-3 py-2 shadow-sm transition-all group-hover:shadow-brand-sm group-hover:border-brand-100">
                        <div className="flex items-center gap-2"><span>{step.icon}</span><span className="text-[10px] font-medium text-midnight-500">{step.label}</span></div>
                      </div>
            ...
            {[
              { icon: Bot, gradient: 'from-violet-500 to-indigo-600', title: 'AI Assistant', desc: 'AI-powered task descriptions, smart assignment suggestions, and automated status reports.' },
              { icon: Target, gradient: 'from-emerald-500 to-teal-600', title: 'Goals & OKRs', desc: 'Set measurable objectives, track key results, and align projects to organizational goals.' },
              { icon: PieChart, gradient: 'from-brand-500 to-brand-700', title: 'Budget Tracking (ETB)', desc: 'Track project budgets in Ethiopian Birr. Manage expenses, approvals, and financial reports natively.' },
              { icon: Zap, gradient: 'from-rose-500 to-pink-600', title: 'Real-time Collaboration', desc: 'See live updates, cursor presence, real-time comments, and instant notifications across your team.' },
      ...
      <section id="product" className="bg-surface-50 py-24 sm:py-32">
        <div className="mx-auto max-w-7xl px-5 sm:px-6 lg:px-8">
          <Reveal>
            <div className="mx-auto mb-16 max-w-2xl text-center">
              <div className="mb-4 inline-flex items-center gap-2 rounded-full bg-brand-50 px-3.5 py-1.5 border border-brand-100">
                <Layers className="h-3.5 w-3.5 text-brand-500" /><span className="text-xs font-semibold uppercase tracking-wider text-brand-600">Product</span>
              </div>
              <h2 className="mb-4 text-3xl font-extrabold tracking-tight text-midnight-800 sm:text-4xl lg:text-5xl">
                Built for how Ethiopian teams{' '}<span className="bg-gradient-to-r from-brand-600 to-purple-600 bg-clip-text text-transparent">actually work</span>
              </h2>
              <p className="text-lg text-midnight-400">Not another foreign tool with a language pack. Onekof is engineered from the ground up for Ethiopian workflows.</p>
            </div>
          </Reveal>
          <Reveal delay={100}>
            <div className="mx-auto max-w-5xl">
              <div className="mb-8 flex flex-wrap justify-center gap-2">
                {productTabs.map((tab, i) => (
                  <button key={tab.label} onClick={() => setActiveTab(i)} className={`flex items-center gap-2 rounded-xl px-5 py-3 text-sm font-medium transition-all ${activeTab === i ? 'bg-gradient-to-r from-brand-600 to-purple-600 text-white shadow-brand-md' : 'bg-white text-midnight-500 hover:bg-brand-50 hover:text-brand-600 border border-surface-300'}`}>
              ...
              <div className="overflow-hidden rounded-2xl border border-surface-300 bg-white shadow-xl shadow-brand-900/5">
                <div className="grid items-center gap-0 lg:grid-cols-2">
                  <div className="p-8 lg:p-12">
                    <h3 className="mb-3 text-2xl font-bold text-midnight-800">{productTabs[activeTab].content.title}</h3>
                    <p className="mb-6 text-base leading-relaxed text-midnight-400">{productTabs[activeTab].content.desc}</p>
                    <Link href="/auth/signup" className="group inline-flex items-center gap-2 text-sm font-semibold text-brand-600 hover:text-brand-700">Try it free<ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-1" /></Link>
                  </div>
                  <div className="bg-gradient-to-br from-brand-50 to-surface-100 p-6 lg:p-8">
                    <div className="rounded-xl border border-brand-100 bg-white p-4 shadow-sm">
                      {activeTab === 0 && (
                        <div className="space-y-3">
                          {['Backlog','In Progress','Review','Done'].map((col, i) => (
                            <div key={col} className="flex items-center gap-3"><div className={`h-2.5 w-2.5 rounded-full ${['bg-midnight-300','bg-brand-500','bg-amber-500','bg-emerald-500'][i]}`} /><span className="text-xs font-medium text-midnight-600">{col}</span><div className="flex-1 h-2 rounded-full bg-surface-200"><div className={`h-full rounded-full ${['bg-midnight-300','bg-brand-400','bg-amber-400','bg-emerald-400'][i]}`} style={{ width: `${[60,45,25,80][i]}%` }} /></div><span className="text-[10px] text-midnight-300">{[12,8,5,24][i]}</span></div>
                          ))}
                        </div>
                      )}
                      {activeTab === 1 && (
                        <div className="space-y-2">
                          ...
                          <div className="grid grid-cols-7 gap-1">
                            {['ሰ','ማ','ረ','ሐ','አ','ቅ','እ'].map((d) => (<div key={d} className="text-center text-[10px] font-medium text-midnight-300 py-1">{d}</div>))}
                            {Array.from({ length: 28 }, (_, i) => (
                              <div key={i} className={`h-7 rounded-md text-center text-[10px] leading-7 ${i === 5 ? 'bg-brand-600 font-bold text-white' : i === 12 ? 'bg-red-100 font-medium text-red-600' : i === 18 ? 'bg-amber-100 font-medium text-amber-600' : 'text-midnight-500 hover:bg-brand-50'}`}>{i + 1}</div>
                            ))}
                          </div>
                        </div>
                      )}
                      {activeTab === 2 && (
                        <div className="space-y-3">
                          {[{ trigger: 'Task → Review', action: 'Assign reviewer', icon: '👤' },{ trigger: 'Sprint ends', action: 'Generate report', icon: '📊' },{ trigger: 'Bug created', action: 'Alert dev team', icon: '🔔' }].map((rule) => (
                            <div key={rule.trigger} className="flex items-center gap-3 rounded-lg border border-surface-200 p-3"><span className="text-lg">{rule.icon}</span><div className="flex-1"><div className="text-xs font-semibold text-midnight-700">{rule.trigger}</div><div className="text-[10px] text-midnight-300">→ {rule.action}</div></div><div className="h-2 w-2 rounded-full bg-emerald-400" /></div>
                          ))}
                        </div>
                      )}
                      {activeTab === 3 && (
                        <div className="space-y-3">
                          {[{ label: 'Velocity', value: '34 pts/sprint', trend: '+12%' },{ label: 'Completion Rate', value: '87%', trend: '+5%' },{ label: 'Avg. Cycle Time', value: '3.2 days', trend: '-18%' }].map((metric) => (
                            <div key={metric.label} className="flex items-center justify-between rounded-lg border border-surface-200 p-3"><div><div className="text-[10px] text-midnight-300">{metric.label}</div><div className="text-sm font-bold text-midnight-800">{metric.value}</div></div><span className="rounded-full bg-emerald-50 px-2 py-0.5 text-[10px] font-semibold text-emerald-600">{metric.trend}</span></div>
                          ))}
                        </div>
                      )}
                    </div>
      ...
      <section className="relative overflow-hidden py-24 sm:py-28">
        ...
        <div className="pointer-events-none absolute inset-0" style={{ backgroundImage: 'linear-gradient(rgba(255,255,255,0.04) 1px, transparent 1px), linear-gradient(90deg, rgba(255,255,255,0.04) 1px, transparent 1px)', backgroundSize: '48px 48px' }} />
        <div className="relative mx-auto max-w-7xl px-5 sm:px-6 lg:px-8">
          <Reveal>
            <div className="mb-16 text-center"><h2 className="text-3xl font-extrabold text-white sm:text-4xl">Teams ship faster with Onekof</h2><p className="mt-3 text-lg text-midnight-200">Real results from real Ethiopian teams</p></div>
          </Reveal>
          <div className="grid grid-cols-2 gap-6 sm:gap-8 lg:grid-cols-4">
            {[{ value: 500, suffix: '+', label: 'Teams onboarded', icon: Building2 },{ value: 40, suffix: '%', label: 'Faster delivery', icon: TrendingUp },{ value: 99, suffix: '.9%', label: 'Platform uptime', icon: Timer },{ value: 4, suffix: '', label: 'Languages supported', icon: Globe }].map((stat, i) => (
              <Reveal key={stat.label} delay={i * 100}>
                <div className="group rounded-2xl border border-white/10 bg-white/[0.04] p-6 text-center backdrop-blur-sm transition-all hover:border-brand-400/30 hover:bg-white/[0.08] sm:p-8">
                  ...
                  <div className="text-3xl font-extrabold text-white sm:text-4xl"><Counter end={stat.value} suffix={stat.suffix} /></div>
      ...
      <section id="about" className="py-24 sm:py-32">
        <div className="mx-auto max-w-7xl px-5 sm:px-6 lg:px-8">
          <Reveal>
            <div className="mx-auto mb-16 max-w-2xl text-center">
              <div className="mb-4 inline-flex items-center gap-2 rounded-full bg-brand-50 px-3.5 py-1.5 border border-brand-100"><Star className="h-3.5 w-3.5 text-brand-500" /><span className="text-xs font-semibold uppercase tracking-wider text-brand-600">Testimonials</span></div>
              ...
              <Reveal key={t.name} delay={i * 100}>
                <div className="gradient-border group h-full rounded-2xl bg-white p-8 shadow-sm hover:shadow-brand-md">
                  ...
                  <div className="flex items-center gap-3">
                    <div className={`flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-br ${t.gradient} text-sm font-bold text-white shadow-sm`}>{t.initials}</div>
                    <div><p className="text-sm font-semibold text-midnight-800">{t.name}</p><p className="text-xs text-midnight-400">{t.role}</p></div>
                  </div>
                </div>
              </Reveal>
            ))}
      ...
      <section id="pricing" className="relative overflow-hidden py-24 sm:py-32">
        ...
        <div className="relative mx-auto max-w-7xl px-5 sm:px-6 lg:px-8">
          <Reveal>
            <div className="mx-auto mb-12 max-w-2xl text-center">
              <div className="mb-4 inline-flex items-center gap-2 rounded-full bg-brand-50 px-3.5 py-1.5 border border-brand-100"><Gem className="h-3.5 w-3.5 text-brand-500" /><span className="text-xs font-semibold uppercase tracking-wider text-brand-600">Pricing</span></div>
              <h2 className="mb-4 text-3xl font-extrabold tracking-tight text-midnight-800 sm:text-4xl lg:text-5xl">Invest in your team&apos;s productivity</h2>
              <p className="text-lg text-midnight-400">Start free. Upgrade when you&apos;re ready. No surprises.</p>
              <div className="mt-8 inline-flex items-center gap-1 rounded-full bg-white p-1 shadow-sm border border-surface-300">
                <button onClick={() => setActivePricing('monthly')} className={`rounded-full px-5 py-2.5 text-sm font-medium transition-all ${activePricing === 'monthly' ? 'bg-gradient-to-r from-brand-600 to-purple-600 text-white shadow-brand-sm' : 'text-midnight-400 hover:text-midnight-600'}`}>Monthly</button>
                <button onClick={() => setActivePricing('yearly')} className={`rounded-full px-5 py-2.5 text-sm font-medium transition-all ${activePricing === 'yearly' ? 'bg-gradient-to-r from-brand-600 to-purple-600 text-white shadow-brand-sm' : 'text-midnight-400 hover:text-midnight-600'}`}>
                  Yearly{activePricing !== 'yearly' && <span className="ml-1.5 text-[10px] font-bold text-emerald-500">-20%</span>}
                </button>
              </div>
            </div>
          </Reveal>
          <div className="mx-auto grid max-w-5xl gap-6 lg:grid-cols-3">
            {[
              { name: 'Free', desc: 'For small teams getting started', price: 0, yearlyPrice: 0, features: ['Up to 10 users','3 projects','Kanban & list views','Ethiopian calendar','4 language support','Basic reports'], cta: 'Get started free', popular: false },
              { name: 'Pro', desc: 'For growing teams that need more', price: 2500, yearlyPrice: 1999, features: ['Unlimited users','Unlimited projects','All views (Timeline, Table)','Automations & workflows','Budget tracking (ETB)','Goals & OKRs','Advanced analytics','Priority support'], cta: 'Start 14-day free trial', popular: true },
              { name: 'Enterprise', desc: 'For large organizations', price: null, yearlyPrice: null, features: ['Everything in Pro','SSO & SAML','Advanced security','Custom integrations','Dedicated support','SLA guarantee','On-premise option','Custom training'], cta: 'Contact sales', popular: false },
            ].map((plan, i) => (
              <Reveal key={plan.name} delay={i * 100}>
                <div className={`relative h-full rounded-2xl border ${plan.popular ? 'border-brand-300 shadow-brand-lg bg-gradient-to-b from-white to-brand-50/30' : 'border-surface-300 bg-white'} p-8 transition-all hover:shadow-brand-md`}>
                  {plan.popular && (
                    <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 rounded-full bg-gradient-to-r from-brand-600 to-purple-600 px-4 py-1 text-xs font-bold text-white shadow-brand-md flex items-center gap-1.5"><Crown className="h-3 w-3" />Most Popular</div>
                  )}
                  <div className="mb-6"><h3 className="text-lg font-bold text-midnight-800">{plan.name}</h3><p className="mt-1 text-sm text-midnight-400">{plan.desc}</p></div>
                  <div className="mb-6">
                    {plan.price !== null ? (
                      <div className="flex items-baseline gap-1"><span className="text-4xl font-extrabold text-midnight-800">{(activePricing === 'yearly' ? plan.yearlyPrice! : plan.price).toLocaleString()}</span><span className="text-sm text-midnight-400">ETB/user/mo</span></div>
                    ) : (<div className="text-4xl font-extrabold text-midnight-800">Custom</div>)}
                    {plan.price === 0 && <p className="mt-1 text-xs text-midnight-300">Free forever, no card needed</p>}
                  </div>
                  <Link href={plan.name === 'Enterprise' ? '#contact' : '/auth/signup'} className={`mb-8 block rounded-xl px-6 py-3.5 text-center text-sm font-semibold transition-all ${plan.popular ? 'bg-gradient-to-r from-brand-600 to-purple-600 text-white shadow-brand-md hover:shadow-brand-lg hover:brightness-110' : 'border border-surface-300 bg-white text-midnight-600 hover:bg-brand-50 hover:border-brand-200 hover:text-brand-600'}`}>{plan.cta}</Link>
                  <ul className="space-y-3">
                    {plan.features.map((f) => (<li key={f} className="flex items-start gap-2.5 text-sm text-midnight-500"><Check className="mt-0.5 h-4 w-4 flex-shrink-0 text-brand-500" />{f}</li>))}
                  </ul>
                </div>
              </Reveal>
            ))}
      ...
      <section className="border-y border-surface-200 bg-white py-16">
        <div className="mx-auto max-w-7xl px-5 sm:px-6 lg:px-8">
          <Reveal><div className="text-center mb-10"><h3 className="text-2xl font-bold text-midnight-800">Why teams switch to Onekof</h3></div></Reveal>
          <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
            {[
              { from: 'Jira', reasons: ['No Ethiopian calendar support','English-only interface','Overly complex for most teams'] },
              { from: 'Trello', reasons: ['No budget/expense tracking','Limited reporting','No workflow automation'] },
              { from: 'Spreadsheets', reasons: ['No real-time collaboration','No task dependencies','Manual status tracking'] },
            ].map((item, i) => (
              <Reveal key={item.from} delay={i * 100}>
                <div className="gradient-border rounded-xl bg-white p-6 shadow-sm hover:shadow-brand-md">
                  <p className="mb-4 text-sm font-semibold text-midnight-800">Switching from <span className="text-brand-600">{item.from}</span>?</p>
                  <ul className="space-y-2">{item.reasons.map((r) => (<li key={r} className="flex items-start gap-2 text-sm text-midnight-400"><X className="mt-0.5 h-3.5 w-3.5 flex-shrink-0 text-red-400" />{r}</li>))}</ul>
                  <div className="mt-4 border-t border-surface-200 pt-4"><p className="flex items-center gap-2 text-sm font-medium text-brand-600"><Check className="h-4 w-4" />Onekof solves all of these</p></div>
                </div>
              </Reveal>
            ))}
          </div>
        ...
        <div className="mx-auto max-w-3xl px-5 sm:px-6 lg:px-8">
          <Reveal>
            <div className="mb-12 text-center">
              <div className="mb-4 inline-flex items-center gap-2 rounded-full bg-brand-50 px-3.5 py-1.5 border border-brand-100"><MessageSquare className="h-3.5 w-3.5 text-brand-500" /><span className="text-xs font-semibold uppercase tracking-wider text-brand-600">FAQ</span></div>
              <h2 className="text-3xl font-extrabold tracking-tight text-midnight-800 sm:text-4xl">Frequently asked questions</h2>
            </div>
          </Reveal>
          <Reveal delay={100}>
            <div className="rounded-2xl border border-surface-300 bg-white px-6 shadow-sm sm:px-8">
      ...
      <section className="relative overflow-hidden py-24 sm:py-32">
        ...
        <div className="relative mx-auto max-w-4xl px-5 text-center sm:px-6 lg:px-8">
          <Reveal>
            <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-brand-400/20 bg-brand-500/10 px-4 py-2 backdrop-blur-sm"><Sparkles className="h-4 w-4 text-brand-400" /><span className="text-sm font-semibold text-brand-300">Ready to transform your workflow?</span></div>
            <h2 className="mb-6 text-3xl font-extrabold text-white sm:text-4xl lg:text-5xl">
              Start managing projects{' '}<span className="bg-gradient-to-r from-brand-400 via-purple-400 to-brand-300 bg-clip-text text-transparent">the Ethiopian way</span>
            </h2>
            <p className="mx-auto mb-10 max-w-2xl text-lg text-midnight-200">Join hundreds of teams already using Onekof to plan, track, and deliver projects faster. Free to start, powerful to scale.</p>
            <div className="mx-auto max-w-lg">
              <div className="flex rounded-2xl border border-white/10 bg-white/[0.06] p-1.5 backdrop-blur-sm">
                <input type="email" placeholder="Enter your work email" className="flex-1 rounded-xl border-0 bg-white px-5 py-3.5 text-sm font-medium text-midnight-800 placeholder-midnight-300 shadow-sm focus:outline-none focus:ring-2 focus:ring-brand-500" />
                <button className="ml-1.5 rounded-xl bg-gradient-to-r from-brand-600 to-purple-600 px-6 py-3.5 text-sm font-semibold text-white shadow-brand-md transition-all hover:shadow-brand-lg hover:brightness-110 active:scale-[0.98]">Get started free</button>
              </div>
              <div className="mt-4 flex flex-wrap items-center justify-center gap-x-6 gap-y-2">
                {['Free forever','No credit card','Setup in 2 min'].map((text) => (
                  <span key={text} className="flex items-center gap-1.5 text-xs text-midnight-300"><CheckCircle2 className="h-3.5 w-3.5 text-brand-400/60" />{text}</span>
                ))}
              </div>
            </div>
          </Reveal>
      ...
      {/* ═══ FOOTER ═══ */}
      <footer className="border-t border-white/5 bg-midnight-900">
        <div className="mx-auto max-w-7xl px-5 py-16 sm:px-6 lg:px-8">
          <div className="grid gap-12 sm:grid-cols-2 lg:grid-cols-5">
            <div className="lg:col-span-2">
              <div className="flex items-center gap-2.5">
                <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-brand-600 to-purple-600 shadow-brand-sm"><span className="text-sm font-black text-white">O</span></div>
                <span className="text-lg font-bold text-white">Onekof</span>
              </div>
              <p className="mt-4 max-w-xs text-sm leading-relaxed text-midnight-200">Modern project management built for Ethiopian teams. Native calendar, local languages, and workflows designed for how you actually work.</p>
              <div className="mt-6 flex gap-3">
                {['Twitter','LinkedIn','Telegram'].map((social) => (
                  <a key={social} href="#" className="flex h-9 w-9 items-center justify-center rounded-xl bg-white/5 text-xs font-bold text-midnight-300 transition-all hover:bg-brand-600/20 hover:text-brand-400 border border-white/5">{social[0]}</a>
                ))}
              </div>
            </div>
            {[{ title: 'Product', links: ['Features','Pricing','Integrations','Changelog','Roadmap'] },{ title: 'Company', links: ['About','Blog','Careers','Press','Contact'] },{ title: 'Resources', links: ['Documentation','Help Center','API Reference','Community','Status'] }].map((col) => (
              <div key={col.title}>
                <h4 className="mb-4 text-xs font-semibold uppercase tracking-wider text-midnight-300">{col.title}</h4>
                <ul className="space-y-3">{col.links.map((link) => (<li key={link}><a href="#" className="text-sm text-midnight-300 transition-colors hover:text-brand-400">{link}</a></li>))}</ul>
              </div>
            ))}
          </div>
          <div className="mt-12 flex flex-col items-center justify-between gap-4 border-t border-white/5 pt-8 sm:flex-row">
            <p className="text-xs text-midnight-400">&copy; {new Date().getFullYear()} Onekof. All rights reserved.</p>
            <div className="flex gap-6">{['Privacy','Terms','Cookies'].map((link) => (<a key={link} href="#" className="text-xs text-midnight-400 transition-colors hover:text-brand-400">{link}</a>))}</div>
          </div>
        </div>
      </footer>
    </div>
